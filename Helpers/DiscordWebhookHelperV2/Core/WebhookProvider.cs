...
using RFRocketLibrary.Helpers.DiscordWebhookHelperV2.Rest;
...
namespace RFRocketLibrary.Helpers.DiscordWebhookHelperV2.Core
{
    /// <summary>
    ...
    public sealed class WebhookProvider : IDisposable
    {
        ...
        /// <summary>
        ///     Creates a webhook with url without binding to the provider,
        ...
        /// <summary>
        ///     Creates a webhook instance without binding to the provider,
        ///     these webhooks do not have logging.
        /// </summary>
        /// <param name="id">
        ///     Webhook id.
        ...
        private static IWebhook CreateWebhook(ulong id, string token, WebhookProvider? provider)
        {
            ...
            Webhook webhook = new(provider, id, token, string.Format(WebhookBaseUrl, id, token));
            provider?._webhooks.Add(webhook.Id, webhook);
        ...
        {
            var allow = !_webhooks.ContainsKey(webhook.Id);
            if (allow)
                _webhooks.Add(webhook.Id, webhook);
